...
async function endLocalMatchPressed() {
  try {
    ...
    const msg =
      (t("modals.endMatch.confirm") || "هل تريد إنهاء المباراة؟");
    const title =
      (t("modals.endMatch.title") || t("buttons.endMatch") || "إنهاء المباراة");

    const yesLabel =
      (t("actions.endMatch") || t("buttons.endMatch") || "إنهاء المباراة");
    const noLabel =
      (t("actions.cancel") || t("modals.cancel") || "إلغاء");
    ...
    const href = (location.pathname || "").includes("/pages/") ? "mode.html" : "pages/mode.html";
    try { location.replace(href); } catch (_) { try { location.href = href; } catch (_) {} }
  } catch (e) {
    try { console.warn("endLocalMatchPressed failed:", e); } catch (_) {}
    try {
      const msg = (typeof t === "function" ? (t("modals.endMatch.confirm") || "") : "") || "هل تريد إنهاء المباراة؟";
      if (confirm(msg)) {
        try { SessionGame.clear(); } catch (_) {}
        const href = (location.pathname || "").includes("/pages/") ? "mode.html" : "pages/mode.html";
        try { location.replace(href); } catch (_) { try { location.href = href; } catch (_) {} }
...
function bindUI() {
  ...
  try {
    ...
    if (endBtn) {
      const isOnline = !!(window.Online && window.Online.isActive);
      const isSpectator = !!(document.body && document.body.classList && document.body.classList.contains("z-spectator"));
