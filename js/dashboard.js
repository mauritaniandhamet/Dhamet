...
async function _fetchLeaderboard(limit = 200) {
  const db = firebase.database();
  const ref = db.ref("leaderboardV1").orderByChild("sortKey").limitToFirst(limit);
...
async function _fetchProfiles(uids) {
  ...
  const reads = uids.map(uid =>
    db.ref("profiles").child(uid).once("value").then(s => s.val() || {}).catch(() => ({}))
  );
  return Promise.all(reads);
}

function _buildLeaderboardBody(items) {
  const wrap = document.createElement("div");
...
function openLeaderboardModal(session) {
  const body = document.createElement("div");
  ...
  (async () => {
    try {
      ...
      const rows = await _fetchLeaderboard(200);
      const uids = rows.map(r => r.uid).filter(Boolean);

      const profiles = await _fetchProfiles(uids);
      ...
      // Replace modal body content in-place.
      body.innerHTML = "";
      body.appendChild(_buildLeaderboardBody(items));
    } catch (e) {
...
function openDeleteAccount(session){
  ...
  function bind(session){
    ...
    const bLb = qs("#btnShowLeaderboard");
    ...
    if (bLb) bLb.addEventListener("click", function(){ openLeaderboardModal(session); });
  }
